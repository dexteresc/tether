...
>(
  ...
  const db = await getTetherDb()
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return db.getAllFromIndex(storeName as StoreName & string, indexName as any, query, limit) as any
}
...
>({
  ...
  limit,
  range,
}: PaginateOptions<StoreName, IndexName>): Promise<Array<TetherDbSchema[StoreName]['value']>> {
  const db = await getTetherDb()
