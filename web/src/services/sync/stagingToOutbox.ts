...
export async function commitStagedToOutbox(): Promise<{
  ...
  for (const [, inputStagedRows] of stagedByInputId) {
    try {
      ...
      const outboxWriteTx = db.transaction(
        "outbox_transactions",
        "readwrite"
      );
      ...
      const stagedWriteTx = db.transaction(
        "staged_extractions",
        "readwrite"
      );
      await Promise.all(
...
export async function commitStagedForInput(
  inputId: string
): Promise<void> {
  const db = await getTetherDb();
  const stagedRows = await db.getAllFromIndex(
    "staged_extractions",
    "by_input_id",
    inputId
  );
