...
function validateEntity(row: EntityInsert): ValidationError[] {
  ...
  if (!row.type) {
    ...
    !(ENTITY_TYPES as readonly string[]).includes(row.type)
  ) {
    errors.push({
      field: "type",
...
function validateIdentifier(row: IdentifierInsert): ValidationError[] {
  ...
  if (!row.type) {
    ...
    !(IDENTIFIER_TYPES as readonly string[]).includes(row.type)
  ) {
    errors.push({
      field: "type",
...
function validateRelation(row: RelationInsert): ValidationError[] {
  ...
  if (!row.type) {
    ...
    !(RELATION_TYPES as readonly string[]).includes(row.type)
  ) {
    errors.push({
      field: "type",
      message: `Invalid relation type. Must be one of: ${RELATION_TYPES.join(", ")}`,
    });
  }
...
function validateIntel(row: IntelInsert): ValidationError[] {
  ...
  if (!row.type) {
    ...
    !(INTEL_TYPES as readonly string[]).includes(row.type)
  ) {
    errors.push({
      field: "type",
      message: `Invalid intel type. Must be one of: ${INTEL_TYPES.join(", ")}`,
    });
  }
  ...
  if (!row.occurred_at) {
    errors.push({
      field: "occurred_at",
      message: "Occurred at date is required",
    });
  } else {
  ...
  if (
    row.confidence &&
    !(CONFIDENCE_LEVELS as readonly string[]).includes(row.confidence)
  ) {
    errors.push({
      field: "confidence",
...
function validateIntelEntity(
  row: IntelEntityInsert
): ValidationError[] {
  const errors: ValidationError[] = [];
...
function validateSource(row: SourceInsert): ValidationError[] {
  ...
  if (!row.reliability) {
    ...
    !(RELIABILITY_LEVELS as readonly string[]).includes(
      row.reliability
    )
  ) {
    errors.push({
