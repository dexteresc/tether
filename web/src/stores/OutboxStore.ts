...
export class OutboxStore {
  ...
  async refresh(): Promise<void> {
    ...
    const rows = await db.getAllFromIndex(
      "outbox_transactions",
      OUTBOX_INDEXES.byCreatedAt
    );

    runInAction(() => {
      this.items = rows;
      this.loaded = true;
      this.syncStatus.setPendingOutboxCount(
        this.items.filter((t) => t.status === "pending").length
      );
    });
  }
  ...
  async updateStatus(
    txId: string,
    status: OutboxStatus,
    patch?: Partial<OutboxTransaction>
  ): Promise<void> {
    const db = await getTetherDb();
    const existing = await db.get("outbox_transactions", txId);
    if (!existing) return;
    await db.put("outbox_transactions", {
      ...existing,
      ...patch,
      status,
    });
