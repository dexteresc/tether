...
export class NLQueueStore {
  ...
  startProcessor(): void {
    ...
    this.processQueue(this.processorAbortController.signal).catch(
      (error) => {
  ...
  private async processQueue(signal: AbortSignal): Promise<void> {
    try {
      while (!signal.aborted) {
        ...
        const pendingItem = this.items.find(
          (i) => i.status === "pending"
        );
  ...
  private async processItem(
    ...
    try {
      ...
      const response: ClassifiedExtractionResponse =
        await llmClient.extract({
          text: item.text,
      ...
      const stagedIds = await mapExtractionToStagedRows(
        item.input_id,
        response,
        null
      );
