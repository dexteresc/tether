...
import type { TableName, OutboxOperation, OutboxTransaction, RemoteRow, ReplicaMeta, ReplicaRow } from '../../lib/sync/types'
...
export async function localCommit<T extends TableName>(params: LocalCommitParams<T>): Promise<OutboxTransaction<T>> {
  const db = await getTetherDb()
  const now = params.now ?? nowIso()
  const txIdFactory = params.txIdFactory ?? defaultTxId

  const txId = txIdFactory()
  const idbTx = db.transaction([params.table, 'outbox_transactions'], 'readwrite')

  const existing = await idbTx.objectStore(params.table).get(params.record_id)
  const existingMeta = existing?.__meta
  const baseUpdatedAt = params.op === 'insert' ? null : (existing?.updated_at ?? null)
  ...
  const mergedRow = {
    ...
    deleted_at: params.op === 'delete' ? now : (params.payload as { deleted_at?: string | null }).deleted_at ?? null,
  }

  const nextMeta: ReplicaMeta = {
    ...ensureReplicaMeta(existingMeta),
    local_last_accessed_at: now,
    local_dirty: params.op !== 'insert' ? true : true,
