...
export interface PullRemote {
  fetchChanges<T extends TableName>(
    table: T,
...
export class SupabasePullRemote implements PullRemote {
  async fetchChanges<T extends TableName>(
    table: T,
    sinceUpdatedAt: string,
    limit: number
  ): Promise<ChangeBatch<T>> {
...
export async function getTableCursorUpdatedAt(
  table: TableName
): Promise<string> {
...
export async function setTableCursorUpdatedAt(
  table: TableName,
  cursor: string
): Promise<void> {
  const db = await getTetherDb();
...
export async function applyChangeBatch<T extends TableName>(
  batch: ChangeBatch<T>
): Promise<void> {
  const db = await getTetherDb();
