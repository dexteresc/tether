...
export async function mapExtractionToStagedRows(
  ...
  for (const entityExt of extraction.entities) {
    ...
    if (extraction.resolutions) {
      const resolution = extraction.resolutions.find(
        (r) => r.entity_ref === entityExt.name
      );
      if (
        resolution &&
    ...
    if (shouldCreateEntityRow) {
      ...
      const entityStaged = await createStagedExtraction(
        inputId,
        "entities",
        entityRow,
        originLabel
  ...
  for (const relExt of extraction.relations) {
    ...
    if (sourceId && targetId) {
      ...
      const relationStaged = await createStagedExtraction(
        inputId,
        "relations",
        relationRow,
        originLabel
  ...
  for (const intelExt of extraction.intel) {
    ...
    const intelStaged = await createStagedExtraction(
      inputId,
      "intel",
      intelRow,
      originLabel
    ...
    for (const entityName of intelExt.entities_involved) {
      ...
      if (entityId) {
        ...
        const intelEntityStaged = await createStagedExtraction(
          inputId,
          "intel_entities",
          intelEntityRow,
          originLabel
