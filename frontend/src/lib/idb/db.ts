    ...
    upgrade(db) {
      for (const table of REPLICA_TABLES) {
        if (!db.objectStoreNames.contains(table)) {
          ...
          store.createIndex(REPLICA_INDEXES.byLocalDirty, '__meta.local_dirty')
          store.createIndex(REPLICA_INDEXES.byLocalLastAccessedAt, '__meta.local_last_accessed_at')
        }
      }

      if (!db.objectStoreNames.contains('outbox_transactions')) {
        const store = db.createObjectStore('outbox_transactions', { keyPath: 'tx_id' })
        ...
        store.createIndex(OUTBOX_INDEXES.byTableRecord, ['table', 'record_id'])
      }

      if (!db.objectStoreNames.contains('nl_input_queue')) {
        const store = db.createObjectStore('nl_input_queue', { keyPath: 'input_id' })
      ...
      if (!db.objectStoreNames.contains('staged_extractions')) {
        const store = db.createObjectStore('staged_extractions', { keyPath: 'staged_id' })
      ...
      if (!db.objectStoreNames.contains('conflict_log')) {
        const store = db.createObjectStore('conflict_log', { keyPath: 'conflict_id' })
        store.createIndex(CONFLICT_INDEXES.byStatus, 'status')
        store.createIndex(CONFLICT_INDEXES.byTableRecord, ['table', 'record_id'])
      }
